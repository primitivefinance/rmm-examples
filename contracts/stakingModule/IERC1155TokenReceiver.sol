...
/// @notice   Contract receiving ERC1155 tokens must implement this interface
...
interface IERC1155TokenReceiver {
    ...
    /// @dev               An ERC1155-compliant smart contract MUST call this function on the token recipient contract, at the end of a
    ...
    /// @param _operator   Address which initiated the transfer (i.e. msg.sender)
    ...
    /// @param _operator   Address which initiated the batch transfer (i.e. msg.sender)
    /// @param _from       Address which previously owned the token
    /// @param _ids        Array containing ids of each token being transferred (order and length must match _values array)
    /// @param _values     Array containing amounts of each token being transferred (order and length must match _ids array)
    /// @param _data       Additional data with no specified format
